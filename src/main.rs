...
fn main() -> io::Result<()>{
    ...
    option_map.insert(11u16, vec!["123412341234123412341234".into()]);
    option_map.insert(u16::from(OptionEnum::UriHost), vec!["coap.me".into()]);
    let msg = CoAPFrame {
        header: get_h1,
    ...
    let frame = CoAPFrame::from_bytes(res.to_vec());
    println!("{:?}", frame);
...
impl CoapOption {
    ...
    fn decode(data: &[u8]) -> Option<CoapOption> {
        ...
        let number = if delta < 13 {
            delta as u16
        } else if delta == 13 && data.len() > 1 {
            13 + data[1] as u16
        } else {
            269 + u16::from_be_bytes([data[1],data[2]])
        };
        
        let value = if vl < 13 {
            &data[(len - vl as usize)..]
        } else if vl == 13 {
            let val_len;
            if delta < 13 {
                val_len = 13 + data[1] as u16;
            }else if delta == 13 {
                val_len = 13 + data[2] as u16;
            }else {
                val_len = 13 + data[3] as u16;
            }
            &data[(len - val_len as usize)..]
        }else {
            let val_len;
            if delta < 13 {
                val_len = 269 + u16::from_be_bytes([data[1],data[2]]);
            }else if delta == 13 {
                val_len = 269 + u16::from_be_bytes([data[2],data[3]]);
            }else {
                val_len = 269 + u16::from_be_bytes([data[3],data[4]]);
            }
            &data[(len - val_len as usize)..]
        };
        Some(CoapOption { number, value: value.to_vec()})
    }
...
impl Header {
    ...
    fn from_bytes(data: &[u8]) -> Option<Header> {
        ...
        Some(Header {ver, msg_type, tkl, code, msg_id })
    }
}
...
impl CoAPFrame {
    ...
    pub fn from_bytes(bytes: Vec<u8>) -> Self {
                ...
                loop {
                    ...
                    if delta < 13 {
                        ...
                        if vl < 13 {
                            ...
                            length = 269 + u16::from_be_bytes([bytes[offset + 1], bytes[offset + 2]]) as u16;
                        ...
                        if vl < 13 {
                            ...
                            length = 269 + u16::from_be_bytes([bytes[offset + 2], bytes[offset + 3]]) as u16;
                            offset = offset + 4 as usize;
                        }
                    } else {
                        number = number + 269 + u16::from_be_bytes([bytes[offset + 1], bytes[offset + 2]]) as u16;
                        ...
                        if vl < 13 {
                            ...
                            length = 269 + u16::from_be_bytes([bytes[offset + 3], bytes[offset + 4]]) as u16;
        ...
        CoAPFrame { header, token: token.to_vec().into(), options, ff: 0xFF, payload: payload.into() }
    }
